...
// In blobify.c - add these function implementations
...
bool blb_blob_put(blb_blob_t *blob, uint8_t value) {
    ...
    // Check if cursor is valid (not -1)
    if (blob->cursor->offset < 0) {
        return false;
    }
    
    // Check if cursor position is within the range
    if (!blb_range_in(blob->range, blob->cursor->offset)) {
        return false;
    }
    
    // Put the value at the cursor position in the block
...
bool blb_blob_move(blb_blob_t *blob, int32_t offset) {
...
int main(void) {
    ...
    blb_blob_t *blob = blb_blob_create(100, 1);
    if (!blob) {
        fprintf(stderr, "Failed to create blob\n");
        return 1;
    }
    
    blb_blob_move(blob, 10);
    
    blb_blob_put(blob, 0xAA);
    
    uint8_t value;
    if (blb_blob_get(blob, &value)) {
        printf("Value at cursor: 0x%02X\n", value);
    }
    
    blb_blob_move(blob, 5);
    
    blb_blob_put(blob, 0xBB);
